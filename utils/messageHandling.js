...
module.exports = async (client, message) => {
    ...
    const total = (await client.databaseManagers.stats.get("phishing_attempts")).count;
    ...
    if (flagged.length > 0) {
        ...
        if (guild) {
            ...
            if (channel) {
                ...
                await warning.reply(
                    `Was this warning accurate? Please report the phishing URL${matches.length > 1 ? "s" : ""} by doing \`p!report ${matches[0]}\``
                );
            } else {
                return await message.channel.send({
                    embeds: [client.newError("Log channel not found! Add one with the command: `p!logs [id]`")],
